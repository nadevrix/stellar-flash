...
import { EXPERT, ms, ago, short, fmtStroops } from '../lib/format.ts';
  ...
  return (
    <div className="px-5 py-5">
      <div className="flex h-16 items-end gap-[3px]">
        {points.map((p, i) => {
          const age = p.ledgerAgeSec ?? 0;
          const h = p.status === 'DOWN' ? 100 : Math.min(100, 22 + age * 5);
...
export function Explorer() {
  ...
  const idle = !!stats && stats.l2.txs === 0;
  const p50 = stats
    ? (idle ? (stats.l2.lifetime?.latencyP50Us ?? null) : stats.l2.latencyP50Us)
    : null;
  const lastPay = txs?.[0]?.createdAt;
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <Card className="mb-6 grid overflow-hidden sm:grid-cols-2 lg:grid-cols-4">
        <StatTile
          label="Confirmation p50"
          value={p50 === null ? '—' : ms(p50)}
          sub={idle ? (p50 !== null ? 'lifetime · no payments in 60s' : 'idle') : (stats ? `p99 ${ms(stats.l2.latencyP99Us)}` : undefined)}
        />
        <div className="border-t border-border sm:border-t-0 sm:border-l">
          <StatTile
            label="Payments / sec"
            value={stats ? stats.l2.txsPerSec.toFixed(2) : '—'}
            sub={idle
              ? (lastPay ? `idle · last ${ago(lastPay)} ago` : 'idle')
              : (stats ? `${stats.l2.txs} in ${stats.windowSec}s` : undefined)}
          />
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.15fr_1fr]">
        <Card>
          ...
          {!txs ? (
            <p className="px-5 py-8 text-sm text-muted">Loading payments…</p>
          ) : txs.length === 0 ? (
  ...
  return (
    <li className="flex items-center justify-between gap-4 px-5 py-3 transition hover:bg-surface">
      ...
      <Link to={`/tx/${t.id}`} className="flex shrink-0 items-center gap-4 text-right">
        <span className="font-mono text-sm tabular-nums">{fmtStroops(t.amount)}</span>
